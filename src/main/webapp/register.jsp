...
<body>
	<form id="go_index" action="index" method="post"></form>
				...
				<form id="login_form" class="form-signin" role="form">
					...
					<div class="form-group has-feedback">
						<input type="text" class="form-control" id="name_input"
							name="name_input" placeholder="请输入账号" autofocus /> 
					...
					<button class="btn btn-lg btn-success btn-block" id="login_btn">注册</button>
				</form>
			</div>
		</div>
	</div>
			...
			$("#login_btn").click(function(){
				...
				$.ajax({
					...
					success : function(result){
						...
						if(result.code===200) {
							showValidateMsg("#login_form","#name_input","success",result.extend.va_msg);
						}else {
							if(result.extend.va_msg === "密码格式不合法！"){
